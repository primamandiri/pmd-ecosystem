    ...
    <div className="p-3 max-w-3xl mx-auto">
      ...
      {Object.entries(grouped).map(([date, salesGroup]: [string, any]) => (
        <div key={date} className="bg-white rounded-xl shadow-sm border border-gray-100 mb-2 overflow-hidden">
          {/* HEADER TANGGAL - klik untuk expand */}
          ...
          {openDate === date && Object.entries(salesGroup).map(([salesName, visits]: [string, any]) => (
            <div key={salesName} className="border-t">
              {/* HEADER SALES - klik untuk expand */}
              ...
              {openSales === salesName && visits.map((v: any) => (
                <div key={v.id}>
                  <button onClick={() => setDetail(detail === v.id ? null : v.id)}
                    className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 text-left border-t border-gray-50">
                    <img src={v.selfie_url} className="w-9 h-9 rounded-full object-cover border" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-800">{v.customer_name}</p>
                      <p className="text-[10px] text-gray-400">🕐 {v.time?.slice(0,5)} | {v.ketemu}</p>
                    </div>
                    <span className="text-[10px] text-gray-400">{detail === v.id ? "▲" : "▼"}</span>
                  </button>

                  {detail === v.id && (
                    <div className="px-4 py-2 bg-gray-50 space-y-1 text-xs border-t">
                      <a href={`https://www.google.com/maps?q=${v.latitude},${v.longitude}`} target="_blank"
                        className="text-blue-600 underline text-[10px]">📍 {v.latitude?.toFixed(6)}, {v.longitude?.toFixed(6)}</a>
                      <p><span className="font-medium">Ketemu:</span> {v.ketemu}</p>
                      <p><span className="font-medium">Bayar:</span> {
                        v.bayar === "Y" ? `✅ Rp ${(v.nominal_bayar||0).toLocaleString("id-ID")} (${v.metode_bayar})`
                        : v.bayar === "T" ? `📅 Janji: ${v.janji_bayar||"-"}`
                        : v.bayar === "N" ? "🚫 Tidak ada piutang" : "-"
                      }</p>
                      <p><span className="font-medium">Order:</span> {
                        v.order === "Y" ? `✅ Ya${v.catatan_order ? ": "+v.catatan_order : ""}`
                        : v.order === "T" ? `❌ Tidak (${v.alasan_order||"-"})`
                        : "-"
                      }</p>
                      ...
                      {v.foto_urls?.length > 0 && (
                        <div className="flex gap-1 mt-1">
                          {v.foto_urls.map((url: string, i: number) => (
                            <img key={i} src={url} className="w-14 h-14 object-cover rounded border" />
                          ))}
                        </div>
                      )}
                    </div>
